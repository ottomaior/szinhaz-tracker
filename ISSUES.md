...
### T-117 · The questionnaire's error red has no token, because the palette has no danger colour
...
### T-118 · landing/README.hu.md is missing two sections its English original has
...
**Resolved on its own, checked 18 September 2026.** The nightly run of the
18th read all three Katona adapters normally (`katona-wp` upserted 35 plays
and 94 performances), so whatever dropped the runner's connections on the
12th has lifted. Left as a record of what it looked like and what the
options were, in case it returns. The same log still showed the Nemzeti
relative `og:image` failing every night, which is why this branch's poster
fix was worth rescuing.
