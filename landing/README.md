...
renders, so nothing on the site names the app’s address. `deploy:landing`
regenerates them first; `npm run render:legal` does it by hand. Until the
operator details in `i18n/legal.ts` are filled in they carry the same
“still being drafted” notice the app’s own routes show.
